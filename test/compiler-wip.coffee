program = ->
  log "<div id='my_output'></div><br /><textarea id='my_source' style='font-size: 13px; font-family: \"Lucida Console\", Monaco, monospace; background-color: black; color: #ddd' rows='10' cols='80'></textarea>"
  input = $('#my_source')
  output = $('#my_output')
  onChanged = ->
    try
      output.html (syntaxed src = window.lastMyInput = input.val()) + '<br><br>'
      compiled src
      output.html output.html() + '✓'
    catch e
      output.html output.html()[...-1] + '✗'
  input.on 'keyup', onChanged
  input.text window.lastMyInput
  onChanged()
  "Ready"

crank = ->
  compiled $('#my_source').val()

run = ->
  eval library + compiled $('#my_source').val()

colorize = (color, string) ->
  "<span style=\"color: #{color}\">#{string}</span>"

printAst = (ast) ->
  if ast? and (Array.isArray(ast) or ast.token)
    copy = if Array.isArray(ast) then [] else {}
    for own n, v of ast when n isnt 'parent'
      copy[n] = printAst v
    copy
  else
    ast
...
"""
  lookup: """
(lookup "Harry" surnames)

surnames [["Michal" "Srb"] ["Harry" "Lachenmayer"]]

lookup (fn [what where]
  (:: a (List (Tuple a b)) (Maybe b))
  (# Find value for a key in list of tuples)
  (match where
    [] [none:]
    (& [key value] rest-where) (if (= what key)
      [just: value]
      (lookup what rest-where))))"""
  quadratic:  """
(defn quadratic [a b c]
  (:: Number Number Number [Number Number])
  (# There is no need for the type signature,
...
fact (fn [n]
  (:: Int Int)
  (# The factorial of n . Recursively calls fact .)
  (if (= n 0)
    1
    (* n (fact (- n 1)))))"""
  ...
  records: """
((def rawInput
  (name: 'Tom country: 'Finland
    age: '34 height: '1.9))

(defn prettify [person]
  update person
    age: (readInt (: age person))
    height: (readFloat (: height person)))

(defn prettify2 [person]
  mutate person
    age: readInt
    height: readFloat)


(def input (prettify rawInput)))"""
  test: """
(def a
  (attr: (b: something)))
"""
  speedup: """
(defn speedup [signal n input]
  (-> input
    (/ by: n)
    signal
    (lift (* n))))"""
...
test = """all-map (fn [fun list] (+ (- fun list)))

any-map (fn [fun list] (+ (- fun list)))

elem (fn [what in] (any-map (= what) in))
not-elem (fn [what in] (all-map (!= what) in))"""
